 ...
 function operation(mode,raw,key='contacts'){
  return new Promise((resolve,reject)=>{
   ...
   try{
    ...
    const request=root.indexedDB.open('FC_LA_COUR_RECOVERY',1);
...
(function(root){
 ...
 function backup(){try{requireWrite();ensureCurrent();const data=qaBackupPayload();prototypeDownloadText('FC_LA_COUR_avant_contacts_'+new Date().toISOString().replace(/[:.]/g,'-')+'.json',JSON.stringify(data,null,2),'application/json');backupProof={baseline:review.baseline,actor:actor()};E('fcuApplyConfirm').disabled=false;message('Téléchargement demandé. Vérifiez la sauvegarde dans vos téléchargements avant de confirmer.');}catch(e){catchError(e);}}
 ...
 async function recoveryBackup(){try{
  ...
  const payload=JSON.parse(raw);if(payload.format!=='FC_LA_COUR_FULL_BACKUP'||payload.schemaVersion!==1)throw Error('Sauvegarde de récupération non reconnue.');
  prototypeDownloadText('FC_LA_COUR_recuperation_contacts.json',raw,'application/json');message('Sauvegarde de récupération téléchargée. Sa restauration complète reste disponible dans les outils de sauvegarde du club.');
 }catch(e){catchError(e);}}
 function report(){try{requireRead();const payload={schema:'la-cour-manager/contacts-diagnostic/1',build:'1.22.13',generatedAt:new Date().toISOString(),captureLoaded:!!snapshot,candidateCount:review?.candidateCount??null,changeCount:review?.changes.length??null,codes:diagnostics.map(x=>x.code),canApply:!!review?.canApply};prototypeDownloadText('FC_LA_COUR_diagnostic_contacts.json',JSON.stringify(payload,null,2),'application/json');message('Diagnostic téléchargé : codes de contrôle et compteurs, sans coordonnées ni identité.');}catch(e){catchError(e);}}
