...
(function(root){
 ...
 function run(){
  ...
  test('Annulation bloquée après modification de fiche',()=>{const {db,s}=fixture(),r=C.apply(db,s,C.plan(db,s,'2026'),['email'],'test');r.state.members[0].phone='0699999999';refuses(()=>C.undo(r.state,r.transaction.id,'test'));});
